...
VALUES
  ((SELECT id FROM suppliers WHERE name = 'Luanda Auto Parts'), 'Mann Oil Filter W712/75',    'Mann',   'W712/75',    2500,  8, 'Tomorrow', 'oil filter lubricant filter', 'oil filter engine'),
  ((SELECT id FROM suppliers WHERE name = 'Angola Moto Parts'), 'Bosch Oil Filter P7153',     'Bosch',  'P7153',      3100, 12, 'Today',    'oil filter lubricant filter', 'oil filter engine'),
  ((SELECT id FROM suppliers WHERE name = 'Luanda Auto Parts'), 'Mahle Oil Filter OC611',     'Mahle',  'OC611',      3800,  5, '2 days',   'oil filter original oem',     'oil filter engine'),
  ((SELECT id FROM suppliers WHERE name = 'Import Car Parts'),  'Original VW Oil Filter',     'VW OEM', '06J115403Q', 5200,  2, '5 days',   'oil filter original oem',     'oil filter engine'),
  ((SELECT id FROM suppliers WHERE name = 'Luanda Auto Parts'), 'Hilux Front Shock Absorber', 'KYB',    'KYB334816', 18500,  4, 'Tomorrow', 'shock absorber front strut',  'suspension shock absorber'),
  ((SELECT id FROM suppliers WHERE name = 'Angola Moto Parts'), 'Golf Front Brake Pads',      'Textar', 'TEX2369201', 7200, 10, 'Today',    'brake pads brake shoes',      'brakes brake pads')
ON CONFLICT (supplier_id, reference) DO UPDATE SET
  ...
  delivery_time = EXCLUDED.delivery_time, synonyms = EXCLUDED.synonyms, category_keywords = EXCLUDED.category_keywords,
  active = true;
