...
- **Language split**: code identifiers, DB tables/columns, stored status values, API routes, and JSON payload keys are **English**. The original design intent was for customer-facing content (WhatsApp messages, PDF text, part/catalog data) to stay **Portuguese (Angola)**, which is why the full-text search still uses the `'portuguese'` config. **As of 2026-07-14, this is temporarily overridden: the project is being built and tested in English for now** (`MESSAGE_LOCALE=en` is the active locale in this dev environment). New customer-facing message wording, seed/test catalog data (product names, service names in `db/seed.sql` and `postman/generate-test-inventory.js`), and sample inventory files should be written in English until this is revisited — the `pt` variants in `src/i18n/messages.ts` are kept in sync alongside `en` (not deleted), but `en` is the one actively being iterated on and tested against. Note the full-text search config staying `'portuguese'` while catalog data is now English content is a known mismatch from this override — Portuguese-specific stemming won't apply to English words, though basic lexeme/substring matching still functions; revisit the search config too if this English phase becomes permanent. The two AI vision-extraction prompt bodies (`src/services/ai.service.ts`) are unrelated to this note — they were already switched to English earlier (2026-07-10) for a different reason (Claude reasons more reliably in English on them), since the model never sees customer text at all.
