...
npm run db:clear   # wipe DB data (db/clear.js)
...
### Admin notifications: in-panel alert feed, not WhatsApp pushes

`admin_alerts` (`src/models/alert.model.ts`) replaced the old pattern of pushing a WhatsApp message to `config.admin.staffPhone`. `createAlert(type, orderNumber, message)` is called from `payment.service.ts` at the same two trigger points as before — payment-proof-received and in-person-payment-requested — but now just inserts a row; the admin panel is expected to poll `GET /v1/admin/alerts` (newest first, capped at 100) and call `POST /v1/admin/alerts/:id/read` to dismiss. There is still no email notification — none exists in this codebase and none was added to scope.
