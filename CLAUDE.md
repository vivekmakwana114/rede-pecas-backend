...
- **Language split**: code identifiers, DB tables/columns, stored status values, API routes, and JSON payload keys are **English**. **As of 2026-07-17, customer-facing conversation language is detected fresh from every inbound message**, not stored durably or frozen at first contact: `detectMessageLocale` (`src/utils/greeting.ts`) runs once per message in `processMessageFlow` (`whatsapp.controller.ts`), before any reply is built. It first checks `detectGreetingLocale` — a leading English/Portuguese greeting word ("Hi/Hello/Hey/Good morning/afternoon/evening/day" vs "Olá/Oi/Bom dia/Boa tarde/Boa noite/Tudo bem/Como está") — then, for any other text, scores the message's words against PT/EN word lists (deliberately a plain word list, not a language-detection library, consistent with this app's no-AI deterministic pattern matching); whichever side has strictly more hits wins, and a tie — including a message with no recognizable words at all, e.g. a VIN, a bare digit, a product reference — returns `null` and leaves the conversation's locale untouched rather than guessing. A hit is cached via `sessionService.saveLocale(phone, locale)` (`session.service.ts`) — Redis-backed with the same 4h `SESSION_TTL`/in-memory fallback as every other pending-choice flag in that file — **not** the `customers` table: a `customers.locale` column existed briefly (detected once at first contact, never re-evaluated) but was removed, because that meant a customer who greeted in Portuguese but later typed in English kept getting Portuguese replies for the rest of the conversation. Every customer-facing WhatsApp send across the message pipeline (registration, vehicle ID, product search, payment/order flow) and the proforma/invoice PDFs (`pdf.service.ts`'s `generateProformaPDF`/`sendProformaWhatsApp`/`generatePrimaveraInvoice`/`sendFinalInvoiceWhatsApp`/`generateMockInvoicePDF` all take an explicit `locale` param now, instead of the fixed `t`) resolves through `resolveLocale(phone)`/`resolveMessages(phone)` (`customer.service.ts`) — a session lookup, falling back to `DEFAULT_LOCALE` (`src/i18n/messages.ts`, a hardcoded `'pt'`, no longer sourced from any env var) when nothing's been detected yet for that phone this session. The old `MESSAGE_LOCALE` env var is gone entirely; the fixed `t` export in `messages.ts` is now hardcoded to the English variant and still backs the paths that are deliberately **not** customer-locale-aware: admin-panel/admin-push messages (`adminAuth.service.ts`, and every `t.admin.*` call in `payment.service.ts`/`product.service.ts`) and the supplier delivery notice (`payment.service.ts`) — none of those are "the customer greeting the bot". Also note: the full-text search config is **not** `'portuguese'` (an earlier version of this note claimed it was) — `db/schema.sql`'s `search_vector` and `product.model.ts`'s `OR_TSQUERY` both build on `to_tsvector('english', ...)`/`to_tsquery('english', ...)` as of the 2026-07-14 English catalog-data switch, so Portuguese-specific stemming doesn't apply to either language's search terms today (lexeme/substring matching still works regardless). The two AI vision-extraction prompt bodies (`src/services/ai.service.ts`) are unrelated to any of this — they were already switched to English earlier (2026-07-10) for a different reason (Claude reasons more reliably in English on them), since the model never sees customer text at all.
